...
const conversationSlice = createSlice({
  ...
  reducers: {
    ...
    addAssistantMessage: {
      ...
      prepare(content: string) {
        return {
          payload: { id: nanoid(), role: 'assistant' as Role, content },
        };
      },
    },
    ...
    addMessages: {
      ...
      prepare(messages: Array<Omit<ConversationMessage, 'id'>>) {
        return {
          payload: messages.map(m => ({ id: nanoid(), ...m })),
        };
      },
    },
    ...
    updateMessage(
      ...
      action: PayloadAction<{
        id: string;
        changes: Partial<Omit<ConversationMessage, 'id'>>;
      }>,
    ) {
    ...
    replaceConversation(state, action: PayloadAction<ConversationMessage[]>) {
...
export const selectConversation = (s: { conversation: ConversationState }) =>
